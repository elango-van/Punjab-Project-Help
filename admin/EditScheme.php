...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['schemecode']) && isset($_POST['method'])) {
        ...
        $errors[] =  "Scheme does not exist";
        // header("Location: schemes.php");
    }

    if (empty($schemecode) || empty($method)) {
        $errors[] = "Scheme does not exist";
    } else {
        include "dbconn.php";
        if ($conn->connect_error) {
            ...
            $stmt = $conn->prepare("SELECT * FROM schemes WHERE schemecode = ?");
...
<body>
    <div class="hero_area">
        ...
        <section>
            <div class="form-container">
                <div class="wrapper">
                    <div class="col-md-2">
                        <div id="form-header"><?php echo htmlspecialchars($method); ?> Scheme</div>
                        <div id="user-form">
                            <div class="form-fill">
                                <!-- action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" -->
                                <form method="post" id="alter-form" onsubmit="return validateForm()">
                                    <input type="text" name="schemecode" id="schemecode" hidden value="<?php echo htmlspecialchars($schemecode); ?>" required>
                                    <input type="text" name="method" id="method" hidden value="<?php echo htmlspecialchars($method); ?>" required>
                                    <div class="mb-3">
                                        <label for="schemename" class="form-label">Scheme Name <label class="field-required">*</label></label>
                                        <input type="text" class="form-control" name="schemename" id="schemename" placeholder="Enter Scheme Name" value="<?php echo htmlspecialchars($schemename); ?>" required>
                                    </div>
                                    <?php if (!empty($errors)) { ?>
    ...
    <script>
        function validateForm() {
            var schemecode = document.forms["alter-form"]["schemecode"].value;
            var method = document.forms["alter-form"]["method"].value;
            var schemename = document.forms["alter-form"]["schemename"].value;

            // console.log([schemecode, schemename, method]);

            // console.log([schemecode, method, name, email, department, role]);
