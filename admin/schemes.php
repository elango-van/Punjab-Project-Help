...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // $strategycode = filter_input(INPUT_GET, 'strategy', FILTER_SANITIZE_NUMBER_INT); 
    // $strategycode = filter_input(INPUT_POST, 'strategy', FILTER_SANITIZE_NUMBER_INT); 
    // $schemename = filter_input(INPUT_POST, 'schemename', FILTER_SANITIZE_STRING); 

    $strategycode = filter_input(INPUT_POST, 'strategy', FILTER_SANITIZE_SPECIAL_CHARS);
    ...
    // $strategycode = htmlentities($_POST['strategy'], ENT_QUOTES, "UTF-8");//$_POST['strategy'];
    ...
    // echo $strategycode . '<br>' . $schemename;
    // exit();
    ...
    if (empty($schemename)) {
        ...
        if ($conn->connect_error) {
            ...
            if ($result->num_rows > 0) {
                ...
                $stmt = $conn->prepare("call addScheme(?, ?)");
                $stmt->bind_param("ss", $strategycode, $schemename);
                $stmt->execute();

                $errors[] = "Scheme name added successfully";
            }
        }
    }
    $stmt->close();
    ...
    <div class="hero_area">
        ...
        <section>
            <div class="form-container">
                <div class="wrapper">
                    <div class="col-md-2">
                        <div id="form-header">Add Scheme</div>
                        <div id="user-form">
                            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                <div class="form-fill">
                                    ...
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Scheme Name <label class="field-required">*</label></label>
                                        <input type="text" class="form-control" id="name" name="name" placeholder="Scheme Name" required>
                                    </div>
                                    <?php if (!empty($errors)) { ?>
                    ...
                    <div class="col-md-2">
                        ...
                        <div id="user-list">
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    echo "<tr><td>" . $row["schemename"] . "</td><td><a href='#' onclick=\"go('" . $row["schemecode"] . "','edit')\">Edit</a></td><td><a href='#' onclick=\"go('" . $row["schemecode"] . "','delete')\">Delete</a></td></tr>";
                                }
                                echo "</table>";
                            } else {
