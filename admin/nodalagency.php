...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $agencyname = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_SPECIAL_CHARS);
    $agencyname = htmlspecialchars($agencyname, ENT_QUOTES, 'UTF-8');

    if (empty($agencyname)) {
        $errors[] = "Nodal Agency name is required";
    } else {
        include "dbconn.php";
        if ($conn->connect_error) {
            ...
            $stmt = $conn->prepare("SELECT nodalagencycode FROM nodalagencies WHERE nodalagencyname = ?");
            $stmt->bind_param("s", $agencyname);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result->num_rows > 0) {
                $errors[] = "Nodal Agency name already exists";
            } else{
                $stmt = $conn->prepare("INSERT INTO nodalagencies (nodalagencyname) VALUES (?)");
                $stmt->bind_param("s", $agencyname);
                $stmt->execute();

                $errors[] = "Nodal Agency added successfully";
            }
        }
    }
    ...
    <script>
        ...
        function go(id, action) {
            ...
            if (action === 'edit') {
                ...
                post_to_url('EditNodalAgency.php', {
                    nodalagencycode: id,
                    method: 'Update'
                }, 'post');
            } else {
    ...
    <div class="hero_area">
        ...
        <section>
            <div class="form-container">
                <div class="wrapper">
                    <div class="col-md-2">
                        <div id="form-header">Add Nodal Agency</div>
                        <div id="user-form">
                            <div class="form-fill">
                                <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Nodal Agency Name <label class="field-required">*</label></label>
                                        <input type="text" class="form-control" id="name" name="name" placeholder="Nodal Agency Name" required>
                                    </div>
                                    <?php if (!empty($errors)) { ?>
                    ...
                    <div class="col-md-2">
                        <div id="list-header">Nodal Agency Information</div>
                        <div id="user-list">
                            <?php
                            include 'dbconn.php';
                            ...
                            if ($result->num_rows > 0) {
                                echo "<table class='table' border='1'><tr><th>Nodal Agency Name</th><th>Edit</th><th>Delete</th></tr>";
                                while ($row = $result->fetch_assoc()) {
                                    echo "<tr><td>" . $row["nodalagencyname"] . "</td><td><a href='#' onclick=\"go('" . $row["nodalagencycode"] . "','edit')\">Edit</a></td><td><a href='#' onclick=\"go('" . $row["nodalagencycode"] . "','delete')\">Delete</a></td></tr>";
                                }
                                echo "</table>";
                            } else {
