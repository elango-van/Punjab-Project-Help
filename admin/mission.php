...
$strategy = $missionname = "";
$nodal = 0;
$errors = [];
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $missionname = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_SPECIAL_CHARS);
    ...
    $nodal = filter_input(INPUT_POST, 'nodal', FILTER_SANITIZE_NUMBER_INT);
    $nodal = htmlspecialchars($nodal, ENT_QUOTES, 'UTF-8');
    ...
    }  else if (empty($nodal) || $nodal == -1 || $nodal == "-1") {
        $errors[] = "Nodal is required";
    } else
    // if(count($errors) == 0) 
    {
        include "dbconn.php";
        if ($conn->connect_error) {
            ...
            $stmt = $conn->prepare("SELECT missioncode FROM missions WHERE missionname = ?");
            $stmt->bind_param("s", $missionname);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result->num_rows > 0) {
                $errors[] = "Mission name already exists";
            } else {
                $stmt = $conn->prepare("CALL addMission(?,?,?)");
                $stmt->bind_param("sis", $strategy, $nodal, $missionname);
                $stmt->execute();
                $errors[] = "Mission added successfully";
            }
        }
        $stmt->close();
        $conn->close();
    ...
    <div class="hero_area">
        ...
        <section>
            <div class="form-container">
                <div class="wrapper">
                    <div class="col-md-2">
                        <div id="form-header">Add Mission</div>
                        <div id="user-form">
                            <div class="form-fill">
                                <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                    ...
                                    <div class="mb-3">
                                        <label for="nodal" class="form-label">Nodal Agency<label class="field-required">*</label></label>
                                        <select id="nodal" name="nodal" class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
                                            <option selected value="-1">Select Nodal Agency</option>
                                            <?php
                                            // include 'dbconn.php';

                                            $sql = "SELECT * FROM nodalagencies";
                                            $result = $conn->query($sql);
                                    ...
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Mission Name <label class="field-required">*</label></label>
                                        <input type="text" class="form-control" id="name" name="name" placeholder="Enter Mission Name" required>
                                    </div>
                                    <?php if (!empty($errors)) { ?>
                    ...
                    <div class="col-md-2">
                        <div id="list-header">Mission Information</div>
                        <div id="user-list">
                            <?php
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                // while($row = $result->fetch_assoc()) {
                                //     echo $row["missioncode"] . "<br>";
                                // }<th>Mission Code</th>, <td>" . $row["missioncode"] . "</td>

                                echo "<table class='table' border='1'><tr><th>Mission Name</th><th>Edit</th><th>Delete</th></tr>";
                                while ($row = $result->fetch_assoc()) {
                                    echo "<tr><td>" . $row["missionname"] . "</td><td><a href='#' onclick=\"go('" . $row["missioncode"] . "','edit')\">Edit</a></td><td><a href='#' onclick=\"go('" . $row["missioncode"] . "','delete')\">Delete</a></td></tr>";
                                }
                                echo "</table>";
                            } else {
