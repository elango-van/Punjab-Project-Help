...
<body>
    ...
    if (!isset($_SESSION['user_id'])) {
        echo "<div class=\"alert alert-primary w-auto\" role=\"alert\">Access denied. You do not have permission to access this page.</div>";
    ...
    if (isset($_SESSION['last_activity']) && (time() - $_SESSION['last_activity'] > $timeout)) {
        ...
        echo "<div class=\"alert alert-primary w-auto\" role=\"alert\">Session expires.</div>";
        header("refresh:15;url=users.php");
        exit();
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['missioncode']) && isset($_POST['method'])) {
            ...
            echo "Mission code does not exist";
            header("Location: mission.php");
        }

        if (empty($missioncode) || empty($method)) {
            ...
            $stmt = $conn->prepare("SELECT * FROM missions WHERE missioncode = ?");
    ...
    <div class="hero_area">
        ...
        <section>
            <div class="form-container">
                <div class="wrapper">
                    <div class="col-md-2">
                        <div id="form-header"><?php echo htmlspecialchars($method); ?> Mission</div>
                        <div id="user-form">
                            <div class="form-fill">
                                <!-- action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" -->
                                <form method="post" id="alter-form" onsubmit="return validateForm()">
                                    <input type="text" name="missioncode" id="missioncode" hidden value="<?php echo htmlspecialchars($missioncode); ?>" required>
                                    <input type="text" name="method" id="method" hidden value="<?php echo htmlspecialchars($method); ?>" required>
                                    <div class="mb-3">
                                        <label for="missionname" class="form-label">Mission Name <label class="field-required">*</label></label>
                                        <input type="text" class="form-control" name="missionname" id="missionname" placeholder="Enter mission Name" value="<?php echo htmlspecialchars($missionname); ?>" required>
                                    </div>
                                    <?php if (!empty($errors)) { ?>
    ...
    <script>
        function validateForm() {
            var missioncode = document.forms["alter-form"]["missioncode"].value;
            var method = document.forms["alter-form"]["method"].value;
            var missionname = document.forms["alter-form"]["missionname"].value;

            // console.log([missioncode, missionname, method]);

            // console.log([missioncode, method, name, email, department, role]);
