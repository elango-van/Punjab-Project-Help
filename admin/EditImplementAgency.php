...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['implementcode']) && isset($_POST['method'])) {
        // $implementingagencycode = $_POST['implementcode'];
        $implementingagencycode = filter_input(INPUT_POST, 'implementcode', FILTER_VALIDATE_INT);
        ...
        header("Location: implementingagency.php");
    }

    if (empty($implementingagencycode) || empty($method)) {
        echo "Implementing Agency code does not exist";
        exit();
    } else {
        ...
        $stmt = $conn->prepare("SELECT * FROM implementingagencies WHERE implementingagencycode = ?");
        $stmt->bind_param("s", $implementingagencycode);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $implementingagencyname = $row["implementingagencyname"];
        } else {
            $errors[] = "Implementing Agency name does not exist";
        }
        $stmt->close();
        $conn->close();
    }
} else {
    echo "Implementing Agency does not exist";
    header("Location: implementingagency.php");
}
?>
...
<body>
    <div class="hero_area">
        ...
        <section>
            <div class="form-container">
                <div class="wrapper">
                    <div class="col-md-2">
                        <div id="form-header"><?php echo htmlspecialchars($method); ?> Implementing Agency</div>
                        <div id="user-form">
                            <div class="form-fill">
                                <!-- action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" -->
                                <form method="post" id="alter-form" onsubmit="return validateForm()">
                                    <input type="text" name="implementingagencycode" id="implementingagencycode" hidden value="<?php echo htmlspecialchars($implementingagencycode); ?>" required>
                                    <input type="text" name="method" id="method" hidden value="<?php echo htmlspecialchars($method); ?>" required>
                                    <div class="mb-3">
                                        <label for="implementingagencyname" class="form-label">Implementing Agency Name <label class="field-required">*</label></label>
                                        <input type="text" class="form-control" name="implementingagencyname" id="implementingagencyname" placeholder="Enter Implementing Agency Name" value="<?php echo htmlspecialchars($implementingagencyname); ?>" required>
                                    </div>
                                    <?php if (!empty($errors)) { ?>
    ...
    <script>
        function validateForm() {
            var implementingagencycode = document.forms["alter-form"]["implementingagencycode"].value;
            var method = document.forms["alter-form"]["method"].value;
            var implementingagencyname = document.forms["alter-form"]["implementingagencyname"].value;

// console.log([implementingagencycode, implementingagencyname, method]);

            // console.log([implementingagencycode, method, name, email, department, role]);

            if (implementingagencyname == "") {
                alert("Implementing Agency Name must be filled out");
                return false;
            }
        ...
        function submitForm() {
            ...
            fetch('SubmitImplementAgency.php', {
                    method: 'POST',
                    body: formData
                })
                .then(response => {
    ...
    <script>
        function redirect() {
            window.location = 'implementingagency.php';
        }
    </script>
