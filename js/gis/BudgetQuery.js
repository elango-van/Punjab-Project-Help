...
function createPieChart(data, header) {
    ...
    let g = svg.selectAll(".arc")
        ...
        .on("mousemove", function (d) {
            ...
            div
                .html("Name:" + d.data.Name + "</br>" + "Value:" + d.data.Budget.toLocaleString('en-IN'))
    ...
    svg.selectAll("text").data(pie(data)).enter()
        ...
        .text(function (d) {
            ...
            return d.data.Budget.toLocaleString('en-IN');
        });
}
function dynamicPieChart(data, header, chartobjname, headerobjname) {

    document.getElementById(headerobjname).innerHTML = header;
    document.getElementById(chartobjname).innerHTML = "";
    let margin = { top: 40, left: 40, right: 40, bottom: 40 };
    let width = 350;
    let height = 350;
    let radius = Math.min(width - 100, height - 100) / 2;
    ...
    let arcOver = d3.arc()
        .outerRadius(radius + 50)
        .innerRadius(0);
    let svg = d3.select("#" + chartobjname).append("svg")
        .attr("width", width)
        .attr("height", height)
        .append("g")
        .attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
...
function groupedBarChart(data, header) {
    ...
    let tip = d3.tip()
        ...
        .html(function (d) {
            ...
            return budgetTitle[d.key] + "<br><strong>Value:</strong>" + d.value.toLocaleString('en-IN');
        });
...
function groupWiseSum(array) {
    let result = [];
    array.reduce(function (res, value) {
        if (!res[value.Code]) {
            res[value.Code] = { Code: value.Code, Name: value.Name, Budget: 0, Expenditure: 0, Climate: 0 };
            result.push(res[value.Code])
        }
        res[value.Code].Budget += value.Budget;
        res[value.Code].Expenditure += value.Expenditure;
        res[value.Code].Climate += value.Climate;
...
function displayBudget(data) {
    let chartData = [], caData = [], natureData = [];
    ...
    data.forEach((d) => {
        ...
        tmpData = {};
        tmpData['Code'] = getnerateCode(d.categoryactioncode);
        ...
        tmpData = {};
        tmpData['Code'] = d.nature;
        tmpData['Name'] = d.nature;
        tmpData['Budget'] = d.budgetallocation;
        tmpData['Expenditure'] = d.budgetexpenditure;
        tmpData['Climate'] = d.expendituretca;
        natureData.push(tmpData);

        // d.categoryactioncode
        // d.nature

        // chartData['Code'] = getnerateCode(d.missionname);
        // chartData['Name'] = d.missionname;
        // console.log(chartData[getnerateCode(d.missionname)]);
        // if (chartData[getnerateCode(d.missionname)] === undefined) {
        //     chartData[getnerateCode(d.missionname)] = 0;
        // }
        // chartData[getnerateCode(d.missionname)] += d.budgetallocation;
        // console.log([getnerateCode(d.strategyname), d.strategyname]);
    ...
    let resultChart = groupWiseSum(chartData);
    // console.log(resultChart);

    // let resultCA = groupWiseSum(caData);
    // console.log(resultCA);

    // let resultNature = groupWiseSum(natureData);
    // console.log(resultNature);

    // let result = [];
    // chartData.reduce(function (res, value) {
    //     if (!res[value.Code]) {
    //         res[value.Code] = { Code: value.Code, Name: value.Name, Budget: 0 };
    //         result.push(res[value.Code])
    //     }
    //     res[value.Code].Budget += value.Budget;
    //     return res;
    ...
    // console.log(result)
        // createBarChart(result, 'Mission');
    // 
    groupedBarChart(resultChart, 'Mission');
    createPieChart(resultChart, 'Mission');

    // dynamicPieChart(resultChart, 'Mission', 'piechart', 'pieheader');
    // dynamicPieChart(resultCA, 'Climate Action', 'cachart', 'caheader');
    // dynamicPieChart(resultNature, 'Nature', 'naturechart', 'natureheader');
}
let jsonBudget = [];
async function getCurrentData() {
    ...
    objQueryHeader.innerHTML = "Financial Year " + finyear;
    // console.log(finyear);
...
async function changeFinancialYear(obj) {
    ...
    console.log(selValue)
    if (selValue !== '-1') {
        objQueryHeader.innerHTML = "Financial Year " + selValue;
...
function changeFilter(obj) {
    ...
    if (selValue !== '-1') {
        ...
        if (selValue === 'mission') {
        ...
        else if (selValue === 'nature') {
            jsonBudget.forEach((d) => {
                let tmpData = {};
                tmpData['Code'] = d.nature; //getnerateCode(d.schemename);
                tmpData['Name'] = d.nature;
                tmpData['Budget'] = d.budgetallocation;
                tmpData['Expenditure'] = d.budgetexpenditure;
                tmpData['Climate'] = d.expendituretca;
            ...
            // }, {});
            // console.log(result)
            groupedBarChart(result, 'Nature');
            createPieChart(result, 'Nature');
        }
        ...
        // else if(selValue !== '') {}
        // else if(selValue !== '') {}
        //budgetallocation, budgetexpenditure, budgetbalance
